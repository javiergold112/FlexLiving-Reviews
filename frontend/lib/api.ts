...
function join(path: string) { return `${API_BASE}${path.startsWith('/') ? '' : '/'}${path}`; }
...
export const api = {
  listReviews: (params: Record<string, any> = {}) =>
    http<{ status: string; data: { reviews: any[]; total: number; pages: number } }>(`/reviews${buildQuery(params)}`),
